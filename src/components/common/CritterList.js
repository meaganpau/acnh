...
const TableContainer = styled.div`
    ...
    @media screen and (max-width: 990px) {
        box-shadow: inset -30px 0 25px -20px rgba(0,0,0,.085);
...
const CritterList = ({ data, critter, hemisphere }) => {
    ...
    const columns = critter === 'fish' ? 
        FishColumns(hemisphere, setNameTableFilter, setTimeTableFilter, setMonthTableFilter, filterTime)
         : BugColumns(hemisphere, setNameTableFilter, setTimeTableFilter, setMonthTableFilter, filterTime);
    ...
    return (
        ...
        { props => (
            <Container>
                <Title>{critter} List <span role="img" aria-label="">{critter === 'fish' ? '🐟' : '🐛'}</span></Title>
                <Buttons>
